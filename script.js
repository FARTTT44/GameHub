...
if (labCtx) {
  ...
  /**
   * Introduce additional openings in the maze to create loops and alternative
   * routes. The recursive backtracker produces a perfect maze with exactly
   * one unique path between any two cells. To avoid a single linear route
   * from the start to the exit/keys, this helper randomly removes some
   * interior walls after the initial generation. Removing walls only
   * increases connectivity and never disconnects the maze, so there is no
   * need to re‑check reachability. The number of loops is proportional to
   * the maze size; feel free to tweak the factor for more or fewer loops.
   */
  function addMazeLoops() {
    // Remove a handful of random walls to create additional routes. We avoid
    // opening boundary walls to keep the maze contained.
    const loops = Math.floor((LAB_ROWS * LAB_COLS) / 6);
    for (let i = 0; i < loops; i++) {
      const r = Math.floor(Math.random() * (LAB_ROWS - 1));
      const c = Math.floor(Math.random() * (LAB_COLS - 1));
      const cell = maze[r][c];
      // Determine candidate neighbors where a wall exists
      const options = [];
      // Only consider breaking right and bottom walls to avoid double‑breaking
      if (cell.walls.right) options.push({ wall: 'right', opp: 'left', nr: r, nc: c + 1 });
      if (cell.walls.bottom) options.push({ wall: 'bottom', opp: 'top', nr: r + 1, nc: c });
      if (options.length === 0) continue;
      const pick = options[Math.floor(Math.random() * options.length)];
      cell.walls[pick.wall] = false;
      maze[pick.nr][pick.nc].walls[pick.opp] = false;
    }
  }
  ...
  /**
   * Compute the next step along the shortest path from a hunter to the
   * target. Uses a breadth‑first search to reconstruct the path between
   * two coordinates within the maze. If no path exists (which should not
   * happen in a connected maze), the hunter will remain in place.
   *
   * @param {object} start - Starting position {r,c}
   * @param {object} goal  - Goal position {r,c}
   * @returns {object} The next position the hunter should move to
   */
  function findNextStep(start, goal) {
    // If already at goal, stay put
    if (start.r === goal.r && start.c === goal.c) return { r: start.r, c: start.c };
    const queue = [];
    const visited = Array.from({ length: LAB_ROWS }, () => Array(LAB_COLS).fill(false));
    // parent map to reconstruct path
    const parent = Array.from({ length: LAB_ROWS }, () => Array(LAB_COLS).fill(null));
    queue.push(start);
    visited[start.r][start.c] = true;
    let found = false;
    // BFS loop
    while (queue.length && !found) {
      ...
      const { r, c } = cur;
      const cell = maze[r][c];
      // Explore neighbours according to open walls
      const neighbours = [];
      if (!cell.walls.top)    neighbours.push({ r: r - 1, c });
      if (!cell.walls.bottom) neighbours.push({ r: r + 1, c });
      if (!cell.walls.left)   neighbours.push({ r, c: c - 1 });
      if (!cell.walls.right)  neighbours.push({ r, c: c + 1 });
      for (const nb of neighbours) {
        if (nb.r < 0 || nb.r >= LAB_ROWS || nb.c < 0 || nb.c >= LAB_COLS) continue;
        if (visited[nb.r][nb.c]) continue;
        visited[nb.r][nb.c] = true;
        parent[nb.r][nb.c] = { r, c };
        // If reached goal, break
        if (nb.r === goal.r && nb.c === goal.c) {
          found = true;
          break;
        }
        queue.push(nb);
    ...
    // If no path found, return start
    if (!visited[goal.r][goal.c]) return { r: start.r, c: start.c };
    // Backtrack from goal to start to find next step
    let step = { r: goal.r, c: goal.c };
    while (parent[step.r][step.c] && !(parent[step.r][step.c].r === start.r && parent[step.r][step.c].c === start.c)) {
      step = parent[step.r][step.c];
    }
    // The current 'step' is now adjacent to start
    return step;
  }
  ...
  function resetLab() {
    ...
    do {
      createMazeGrid();
      generateMaze(0, 0);
      // Add extra loops to allow multiple paths through the maze
      addMazeLoops();
      attempts++;
  ...
  function labMovePlayer(dr, dc) {
    ...
    // Move hunters using breadth‑first search to find the shortest path to the player.
    labHunters.forEach((h, idx) => {
      const next = findNextStep({ r: h.r, c: h.c }, playerPos);
      // Only move if next step is adjacent; otherwise stay put
      if (next.r === h.r && next.c === h.c) return;
      // Update hunter position to the next step
      h.r = next.r;
      h.c = next.c;
    });
